...
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'AutoTestKiaan')
BEGIN
    ALTER DATABASE AutoTestKiaan SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE AutoTestKiaan;
END
GO

-- Step 2: Create the AutoTest database
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'AutoTestKiaan')
BEGIN
...
BEGIN
    CREATE LOGIN Auto_user WITH PASSWORD = 'Password123', DEFAULT_DATABASE = AutoTestKiaan;
END
ELSE
BEGIN
    ALTER LOGIN Auto_user WITH PASSWORD = 'Password123', DEFAULT_DATABASE = AutoTestKiaan;
END
GO
